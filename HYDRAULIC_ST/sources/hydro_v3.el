	...
	BODY
		...
		IF(iopt == 1 OR iopt == 2) THEN
			...
			FOR(j IN 1, ntank) 
				...
				--WRITE("Time = %lg\t", time)
				--WRITE("Htank=%lg\n", T[j].H - 10*sin(time))
				--F[node+npipe+j] = Xunk[nd2] - T[j].H + 10*sin(time)
				IF(NOT Transient) THEN
					F[node+npipe+j] = T[j].H - T[j].Ho
					nleqn_system.Jacob.stampItem(node+npipe+j, nd2, 1.)
				ELSE
					F[node+npipe+j] = T[j].H + 0.5*T[j].Q * DT / T[j].A - T[j].H_old + 0.5*T[j].Q_old * DT / T[j].A 
					nleqn_system.Jacob.stampItem(node+npipe+j, nd2, 1.)
					nleqn_system.Jacob.stampItem(node+npipe+j, node+npipe+j, .5*T[j].Q * DT / T[j].A)
				END IF
			END FOR
			--Flow Equations
			FOR(j IN 1, nflow)
